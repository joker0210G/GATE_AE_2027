...
### 🚀 Added & Enhanced
- **Virtual TCS/GATE Scientific Calculator Simulator (`assets/calculator/`):** Built an authentic, interactive, and offline-ready web simulator for the TCS iON scientific calculator used in GATE. Includes full unary post-fix functions ($\sin, \cos, \tan, \sinh, \ln, \log_{10}, x^y, \sqrt[y]{x}, x!, \text{Exp}, \pi, e$), memory registers (`MS`, `MR`, `M+`, `M-`, `MC`) with active `[M]` memory flag indicator, and a strict **Exam Mode toggle (Mouse-Only)** to simulate real TCS test interface conditions.
- **TCS Calculator Master Guide & AIR 1 Topper Tutorial (`00 - META/`):** Authored [`00 - META/GATE TCS Calculator Guide & NAT Precision Rules.md`](00%20-%20META/GATE%20TCS%20Calculator%20Guide%20%26%20NAT%20Precision%20Rules.md) with an embedded live calculator simulator, direct link to the official TCS iON web portal, and a 3-tier tutorial (Beginner post-fix input rules $\to$ Intermediate bracket chaining $\to$ Advanced AIR 1 zero-loss memory register workflows and aerospace-specific calculator traps).
- **NAT Precision & Tolerance Engine Protocol (`AGENTS.md`):** Updated the AI tutor and mock review protocols to automatically compute official IIT Madras tolerance windows ($[V_{\min}, V_{\max}]$) on all Numerical Answer Type questions, perform forensic audits on premature intermediate rounding errors, and diagnose Deg vs. Radian angle mode slips.
- **Trap Spotter & Reverse-Engineering Engine Integration (`AGENTS.md`, `mock-test-reviewer`):** Integrated the Trap Spotter Sandbox directly under the hood of `mode: mock`. Every mock test batch now organically blends standard 5-tier questions with 1–2 flawed-solution reverse-engineering problems without requiring dedicated prompt keywords, while forensic review automatically generates IIT Distractor Trap Blueprints for all MCQ options.
- **Multi-Subject Cross-Pollination Synthesizer (`AGENTS.md`, `mock-test-reviewer`):** Integrated the Cross-Pollination Engine directly under the hood of `mode: mock`. Sectional and mock papers now organically embed interdisciplinary hybrid problems testing boundary condition interactions across Aerodynamics, Propulsion, Flight Mechanics, and Structures without requiring special keyword activation.
- **AIR Rank & Institute Cutoff Simulator (`AGENTS.md`, `prep-analyser`, `00 - META/`):** Integrated a GATE Aerospace (AE) rank and cutoff simulation engine. Maps student practice telemetry into projected raw marks, normalized score (out of 1000), projected AIR bracket, and an Institute Admission & PSU Recruitment Probability Matrix (IISc, IITB, IITM, IITK, IITKgp, DRDO RAC, ISRO, IIST, DIAT). Authored master reference guide [`00 - META/GATE AE AIR Rank & Institute Cutoff Simulator.md`](00%20-%20META/GATE%20AE%20AIR%20Rank%20%26%20Institute%20Cutoff%20Simulator.md).
- **3-Hour Full-Mock Endurance & Fatigue Telemetry (`AGENTS.md`, `mock-test-reviewer`, `prep-analyser`, `00 - META/`):** Implemented the 3-Block Segmented Telemetry framework for 180-minute full mocks (Hour 1: Speed Baseline $\to$ Hour 2: Core Heavy 2M $\to$ Hour 3: Fatigue Resistance & Trap Avoidance). Calculates the Fatigue Degradation Index ($FDI$) and accuracy decay to diagnose pacing bottlenecks and eliminate final 45-minute negative marking drag. Authored [`00 - META/GATE 3-Hour Full-Mock Endurance & Fatigue Protocol.md`](00%20-%20META/GATE%203-Hour%20Full-Mock%20Endurance%20%26%20Fatigue%20Protocol.md).
- **Interactive Spaced Repetition Flashcards (`06 - FORMULA SHEETS/`, `07 - TEMPLATES/`):** Integrated native `[!question]-` foldable click-to-reveal active recall flashcard decks across all subject formula sheets (Aerodynamics, Propulsion, Structures, Flight Mechanics, Mathematics, and General Aptitude). Created centralized navigator [[06 - FORMULA SHEETS/Master Flashcard Hub]], authoring template [`07 - TEMPLATES/Interactive Flashcard Deck Template.md`](07%20-%20TEMPLATES/Interactive%20Flashcard%20Deck%20Template.md), and interactive testbed [`demo.md`](demo.md).
- **Feature Catalog & Roadmap (`FEATURES.md`):** Created a master feature specification and multi-phase roadmap covering active capabilities and future study enhancements.
